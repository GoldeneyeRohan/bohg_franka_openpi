...
def get_task_dataset_paths(task_config, setting="base", ablation="base"):
...
def main(remap_config:str, output_path:str, task_name:str, setting:str, ablation:str, *, cmd_freq:int = 10, push_to_hub:bool = False):
    ...
    for task in config["tasks"].values():
        language_instruction, data_path, exclude_file = get_task_dataset_paths(task, setting, ablation)
        task_remapping = remapping.copy()
        task_remapping["language_instruction"] = language_instruction
        ...
        raw_dataset = DPEpisodeDataset(data_path, task_remapping, exclude_episodes=exclude_episodes)
        raw_datasets.append(raw_dataset)
        print(f"Loaded {raw_dataset.num_episodes}.")

    dummy_episode, ep_len = raw_datasets[0].get_episode(0)
